...
class DeviceDiscovery:
    ...
    def discover(self) -> list[Device]:
        ...
        """
        try:
            # returncode is checked explicitly below
            ...
            raise DeviceDiscoveryError("Timeout while discovering devices") from err
        ...
        try:
            ...
            raise DeviceDiscoveryError(
                f"Failed to parse tailscale status: {err}"
            ) from None
    ...
    def _parse_status(self, status_data: dict[str, Any]) -> list[Device]:
        ...
        # Parse peer devices
        peers_data = (
            status_data.get("Peer")
            or status_data.get("Peers")
            or status_data.get("peers", [])
        )

        peers = peers_data.values() if isinstance(peers_data, dict) else peers_data
    ...
    def _parse_peer(self, peer: dict[str, Any]) -> Device | None:
        ...
        ip = (ip_data[0] if ip_data else None) if isinstance(ip_data, list) else ip_data
        ...
        # Real `tailscale status --json` peers use "DNSName"; older/synthetic
        # schemas may use "Name"/"HostName" instead.
        name = (
            peer.get("DNSName")
            or peer.get("Name")
            or peer.get("name")
        ...
        # Get machine ID - try multiple possible keys (real schema: "MachineID")
        machine_id = peer.get("MachineID") or peer.get("ID") or peer.get("id", "")
